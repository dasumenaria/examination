	...
	<div class="form-body">
		...
		<div class="col-md-3">
			<div class="form-group">
				...
				<select class="select2me form-control" id="stu" tabindex="1" onChange="number()" >
					<option value="">--Select Student--</option>
					<?php
					$set=mysql_query("select `name`,`id` from `student` order by `id`");
					while($fet=mysql_fetch_array($set)) 
					{$k++;
					$student_id_id=$fet['id'];
					$student_name=$fet['name'];
					?>
					<option value="<?php echo $student_id_id; ?>">
...
function number(){
	   ...
	   var stu=$("#stu").val();
	  	...
	  	$.ajax({
			url: "ajax_student_edit.php?sch="+schlr+"&rol="+roll+"&cls="+cls+"&sec="+sec+"&stu="+stu,

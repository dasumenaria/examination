...
header("Content-Disposition: attachment; filename=PF_excel_report.xls");
       ...
       <thead>
								<tr>
									...
									while($frq=mysql_fetch_array($qry))
									{
										...
										$col=0;
										$qt=mysql_query("select `exam_category_type_id` from `exam_mapping` where `class_id`='$class_id' && `section_id`='$sect_id' && `subject_id`='$sub_id'");
									...
									while($frq1=mysql_fetch_array($qry1))
									{
										...
										$ft1=mysql_fetch_array($st1);
										  ...
										  $sst1=mysql_query("select `name` from `master_sub_subject` where `id`='$sub_subject_id'");
										$fst1=mysql_fetch_array($sst1);
										
										$sub_subject_name=$fst1['name'];
										
										
										$col=0;
										$qt1=mysql_query("select `exam_category_type_id` from `exam_mapping` where `class_id`='$class_id' && `section_id`='$sect_id' && `subject_id`='$elec_id'");
									...
									<th style="text-align:center" colspan="<?php echo $col; ?>">
										 <?php echo $elec_name; ?>-<?php echo $sub_subject_name; ?>
								...
								<?php 
								$w=0;
									$qr=mysql_query("select * from `student` where `class_id`='$class_id' && `section_id`='$section_id'");
								...
								<?php
									...
									while($frq=mysql_fetch_array($qry))
									{
										...
										$sub_name=$ft['subject'];
										$col=0;
										$qt=mysql_query("select `exam_category_type_id`,`max_marks` from `exam_mapping` where `class_id`='$class_id' && `section_id`='$sect_id' && `subject_id`='$sub_id'");
									while($fqt=mysql_fetch_array($qt))
									{$col++;
										$exam_type_id=$fqt['exam_category_type_id'];
										$exam_name=$fqt['exam_name'];
									...
									$retrive_type=$fst['id'];
									$value_sub=0;
								//echo "select `".$retrive_type."` from `student_marks` where `scholar_no`='$scholar_no' && `exam_name`='$exam_id' && `subject_id`='$sub_id'";		
									$query=mysql_query("select `id`,`marks` from `student_marks` where `scholar_no`='$scholar_no' && `subject_id`='$sub_id' && `master_exam_type_id`='$exam_type_id'");
									$fetch=mysql_fetch_array($query);
									
									 $value_sub=$fetch[$retrive_type];
									?>
			...
			<?php	///////////////////////////////////////ELECTIVE	./////////////////////	?>				
							...
							<?php
									...
									$value_sub=0;
								//echo "select `".$retrive_type."` from `student_marks` where `scholar_no`='$scholar_no' && `exam_name`='$exam_id' && `subject_id`='$sub_id'";		
									$query=mysql_query("select `marks`,`id` from `student_marks` where `scholar_no`='$scholar_no' && `subject_id`='$sub_id' && `master_exam_type_id`='$exam_type_id'");
									$fetch=mysql_fetch_array($query);
									...
									<td style="text-align:center">
										...
										$sects=mysql_query("select * from `student_result` where `roll_no`='$roll' && `class_id`='$class_id' && `section_id`='$section_id' ");
										$fects=mysql_fetch_array($sects);
										$per=$fects['per'];
										$total=$fects['total'];
										$status=$fects['status'];
										?>

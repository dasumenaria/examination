...
<script>
		...
		$(".user1").live("change",function(){
			...
			if(s.length>0){
				$.ajax({
					url: "ajax_roll_no.php?pon1="+s+"&pon="+t,
					}).done(function(response){
					$("#sec").html(""+response+"");
					});
 			}else
	...
	$(".user2").live("change",function(){
		...
		if(s.length>0){
		$.ajax({
			url: "ajax_roll_no.php?cls="+t+"&sec="+s+"&sub="+v,
			}).done(function(response) {
			$("#exl").html(""+response+"");
		});
		}else
	...
	$(".ust").live("change",function(){
		...
		var v=$(".user2").val();
		$.ajax({
			url: "ajax_roll_no.php?cls="+t+"&sec="+s+"&sub="+v+"&trm="+g,
			}).done(function(response) {
	...
	$(".ust2").live("change",function(){
		...
		$.ajax({
			url: "ajax_roll_no.php?cls="+t+"&sec="+s+"&sub="+v+"&trm="+g+"&cat="+f,
			}).done(function(response) {
